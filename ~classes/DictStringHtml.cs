...
namespace Ans.Net8.Web
{
    ...
    public class DictStringHtml
        : Dictionary<string, HtmlString>
    {

        /* ctors */


        public DictStringHtml(
            string serialize,
            bool useTypograf = false)
        {
            ...
            {
                var a1 = item1.Split('=');
                var key1 = _decode(_unmasking(a1[0]));
            ...
            params string[] items)
        {
            foreach (var item1 in items)
            {
                ...
                Add(a1[0], a1[1].ToHtml(useTypograf));
            }
        }
        ...
        {
            return string.Join(";",
                this.Select(x => $"{_encode(x.Key)}={_encode(x.Value.ToString())}")
                    .ToArray());
        }
